...
version: 2026-09-21
description: End-to-end workflow for new EBM systematic reviews and meta-analyses as well as publication-grade reproduction. Applies current Cochrane Handbook methods and PRISMA reporting with protocol-first gates, dual independent screening/extraction and adjudication, Zotero full-text retrieval and provenance, design-specific risk-of-bias assessment, reproducible synthesis, GRADE/CINeMA certainty, multi-agent execution and independent approval before each stage. Preserves each project's approved figure/table contract and existing publication formats.
---

# 医学循证系统评价、Meta 分析与论文复现工作流

本规范用于两类任务：**新循证综述研究**（从研究问题、方案、检索、筛选、全文、提取、偏倚风险到综合与报告）以及**已发表研究的复现/校准**。出版交付可面向 *Critical Care*, *The Lancet*, *JAMA*, *BMJ*, *NEJM*, *European Heart Journal* 等期刊。新综述必须先依照当前 Cochrane Handbook 形成项目方案；复现任务则遵循后续原刊忠实度要求。
...
的**方法可追溯执行、证据综合、已发表研究复现与出版级交付标准**。新综述运行顺序、agent 职责和阶段放行规则见 [EBM systematic review workflow](../../docs/EBM_SYSTEMATIC_REVIEW_WORKFLOW.md)、[Agent stage gates](../../docs/AGENT_STAGE_GATES.md) 与 [Cochrane requirements map](../../docs/COCHRANE_AGENT_REQUIREMENTS.md)。
...
## 7. 新 EBM 系统评价与 Meta 分析：Cochrane 方法与多 Agent 阶段放行

本节用于用户提出**新的临床问题**，不要求先找一篇论文供复现。当前 Cochrane Handbook 是方法依据；2022 JSLHR tutorial 用于补充实际流程、研究编码、依赖效应、异质性、敏感性与报告示例。不得把教程中的语言学习数据、R 包、效应大小阈值或结论直接套入临床项目。Zotero、AI agent 与下述软件状态机属于项目实现，不是 Cochrane 指定工具。

### 7.1 团队负责人和 Agent 职责

执行者先按阶段建立独立 agent，不得让一个 agent 同时执行并批准自己的阶段。项目至少配置执行角色与两名不同 ID 的复核 agent；筛选和提取本身仍须保留两份独立原始判断/提取，不能用两张“质量审批票”替代。

| 阶段 | 执行 Agent | 独立复核 Agent | 关键产物 |
|---|---|---|---|
| Protocol | 临床方法学家 | 临床内容专家、方法学审查者 | 方案、PICO/PICOTS、资格标准、结局与分析计划、修订日志 |
| Search | 信息检索专家 | 第二检索专家、检索质量审查者 | 各库完整检索式、检索日期/命中数、引文检索、检索同行审查 |
| Deduplicate | 文献数据管理员 | 来源溯源审计者、study/report 链接审查者 | 全量来源记录、重复合并日志、研究-报告映射 |
| Title/abstract | Reviewer A 与 Reviewer B 分别执行 | 仲裁者、筛选审计者 | 两份独立判断、冲突清单、仲裁及理由 |
| Full-text retrieval | Zotero 全文管理员 | 全文获取审查者、身份匹配审查者 | 附件清单、获取状态、待处理队列、哈希与页码索引 |
| Full-text screening | Reviewer A 与 Reviewer B 分别执行 | 仲裁者、资格审查者 | 双份独立资格判断、逐篇排除理由、仲裁记录 |
| Extraction | Extractor A 与 Extractor B 分别执行或一人提取并独立核验 | 数据/来源审计者 | 研究与结局数据、报告位置、差异对账表 |
| Risk of bias | 与设计相匹配的 RoB 评估者 | 第二评估者、方法学审计者 | 按设计、结果和领域记录的判断与依据 |
| Synthesis | 统计分析 Agent | 独立统计员、临床兼容性审查者 | 可执行代码、数据快照、模型诊断、敏感性结果与分析决定 |
| Certainty | GRADE/CINeMA 评估者 | 第二评估者、SoF 一致性审计者 | 关键结局确定性、降级理由、Summary of Findings |
| Reporting | 论文组装 Agent | PRISMA 审查者、复现审查者 | PRISMA 流程/清单、检索附录、手稿与最终审计 |

Agent 必须先读已批准的 `review_protocol.json` 和当前 stage card，只执行分配的阶段。缺失事实标为 `Not reported`、`awaiting classification` 或未解决问题；不得根据常识补值或自动将记录判为纳入。Agent 输出必须列示输入、方法/章节依据、判定、来源坐标、异常、产物路径和待决事项。

### 7.2 严格阶段状态机

顺序为 `protocol -> search -> deduplication -> title_abstract_screening -> fulltext_retrieval -> fulltext_screening -> data_extraction -> risk_of_bias -> synthesis -> certainty -> reporting`。本地 CLI `review-stage` 写入 `verification/agent_stage_ledger.json` 并验证事件哈希链。每阶段状态为 `pending / in_progress / awaiting_review / needs_revision / approved`。

1. 负责人创建执行 agent 并提供该阶段的 stage card、已批准输入、验收标准和输出路径。
2. 执行 agent 提交可复核文件和摘要。提交文件须位于项目目录内，记录 SHA-256。
3. 两个不同 ID 的独立复核 agent 按验收标准检查产物、记录报告与 `approve` / `revise` 票。执行 agent 不能复核自己的阶段。
4. 两票均为 `approve` 才能放行下一阶段。任一 `revise` 票阻断下游，执行 agent 修订后提交新尝试，再由不同 reviewer 复核。
5. 重跑已批准阶段会把该阶段和所有下游阶段的当前审批失效，并保留历史尝试，避免新旧数据混合。

失败门包括：方案定义不完整；检索式或落盘批次不可复现；流量/去重不守恒；筛选、提取或偏倚判断少了独立意见且未仲裁；全文仍缺失却被当成已排除；数值没有报告坐标；综合不符合预设或适合性未论证；统计结果不能由归档数据和代码复算；GRADE/CINeMA 与结果表不一致。失败时只返工当前阶段，不生成下游结论。

### 7.3 新综述方法顺序与验收依据

| 阶段 | 方法边界 | 放行条件 |
|---|---|---|
| Protocol | 明确研究目的、概念/操作定义、研究设计、资格标准、结局重要性/时间点、分析组、缺失数据、亚组与敏感性计划；登记并记录方案修订。Handbook Ch 1-3；tutorial pp. 3-5。 | 方案关键字段可执行；临床兼容性与决策边界明晰；两名独立复核通过。 |
| Search | 多库、主题词+自由词、各库语法分别校验；记录完整策略、来源、检索日期、限制及理由；保存导出批次和策略同行审查。Handbook Ch 4；tutorial pp. 5-7。 | 所有来源与策略可复跑；检索性能审查完成；报告命中数等于落盘账本。 |
| Selection | 研究而非单报告作为综合单位；题录和全文资格判断独立进行；疑难向纳入方向暂存并仲裁；逐条保存理由。Handbook Ch 4.6；tutorial pp. 6-7。 | 双份判断齐全；冲突已仲裁；PRISMA 数量与排除理由相符。 |
| Full text | 以 Zotero collection 管理待获取报告；从本地库/JSON 导出定位附件；记录检索状态、来源路径、SHA-256、全文解析状态和页码位置。未取得全文记为 `awaiting classification`，待进一步处理。Handbook Ch 4.6.3。 | sought / retrieved / assessed 分开核算；无未决全文被伪记为全文排除；附件与题录匹配无未解决歧义。 |
| Extraction | 预先设计且试运行表单；记录研究、参与者、干预/对照、设计、结局数值、资金/冲突；每个数值锚定报告和页/表/图；记录联系作者、换算及假设。Handbook Ch 5；tutorial pp. 7-9。 | 两份独立提取或清晰的独立核验记录；差异关闭；每个合成数据单元都能追溯。 |
| Risk of bias | 按研究设计选择 RoB 2/ROBINS-I 等合适工具；在相关结果/结局层面记录领域证据，不用单一质量总分替代。Handbook Ch 7-8；非随机干预研究按 Ch 25。Ch 9 用于研究特征汇总和综合准备。 | 评估者依据、分歧与仲裁齐全；与敏感性分析/解释的衔接被说明。 |
| Synthesis | 先判断临床、设计、测量和结局是否可比，再选效应量与模型；正确处理多臂、多个相关效应、缺失数据、异质性、影响点、小研究效应和预先计划的调节因素。Handbook Ch 6, 10-11；tutorial pp. 9-15。 | protocol 偏离有理由；分析代码、软件版本、锁定数据和诊断结果归档；独立统计复核复算一致。 |
| Certainty and report | 对关键结局做 GRADE，NMA 按适用规范处理网络确定性；生成 SoF；按 PRISMA 报告来源、检索、筛选、研究特征、综合、偏倚、确定性和限制。Handbook Ch 11,14；tutorial Appendix A pp. 20-21。 | 证据等级理由透明；摘要/正文/SoF 数值一致；最终两个独立复核通过。 |

### 7.4 Zotero 获取后接入筛选和全文分析

```powershell
sci-nma-agent zotero-fulltext --project PROJECT --source ZOTERO_DATA_DIR --collection COLLECTION_NAME --screening-table PROJECT\screening\master_screening_table.xlsx


可用 Zotero `zotero.sqlite` 所在数据目录，或 Zotero JSON 导出。桥接模块只读 SQLite，优先复用已下载附件及 `.zotero-ft-cache`，支持本地 PDF 文本提取，并输出 `original_materials/zotero_fulltext_manifest.json`、`zotero_fulltext_corpus.jsonl`、逐篇文本和页索引。清单必须报告无法匹配、多个附件、无全文、扫描件待 OCR、空文本和解析错误；不得静默选择或丢弃。全文获取状态只用于文献获取对账，不代替人工资格决定。

固定图表格式和项目既定 Office 交付格式保持不变；进入图表生成前，只更新已审定数据和输入来源。
